...
/**
 * this class defines the Projection on the first k components of a one-sorted
 * WN function-tuple F its precise semantics is &lang;X_i,..,X_k&rang; \circle F
 * ;
 *
...
public final class TupleProjection implements FunctionTuple, UnaryOp<FunctionTuple> {
     ...
     * @throws IllegalArgumentException if the specified function is not
     * one-sorted
     */
    ...
    private static int checkSize(final int k, final int arity) {
        if (k < 1 || k > arity) {
    ...
    public Map<Sort, Integer> splitDelimiters() {
        final Map<Sort, Integer> delimiters = this.ftuple.splitDelimiters();
    ...
    public FunctionTuple specSimplify() {
        ...
        if (this.ftuple instanceof TupleProjection tupleProjection) {
            return new TupleProjection(tupleProjection.ftuple, this.k);
        }
        // main case
        if (this.ftuple instanceof Tuple tuple) {
            if (this.ftuple.simplified()) { // VERY IMPORTANT: we assume that the inequation set corresponding to the filter has been shown "satisfiable"...
    ...
    /**
     * @return the minimun of cardinality lower bounds of tuple functions (meant
     * as domains) referred to by inequalities with indices greater than a given
     * value; <code>0</code>, if the cardinality of some functions is undefined
     */
    private static int minLb(final Collection<? extends Equality> eqs, final List<? extends SetFunction> components, final int k) {
        var minlb = Integer.MAX_VALUE;
        for (int x : Guard.indexSet(eqs)) {
            if (x > k) {
                final var card = components.get(x - 1).card();
                if (card == null) {
                    return 0;
                }
                if (minlb > card.lb()) {
    ...
    @Override
    public TupleProjection buildOp(final FunctionTuple arg) {
