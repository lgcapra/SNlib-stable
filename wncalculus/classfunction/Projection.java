 ...
 */
public final class Projection extends ProjectionBased {
    ...
    /**
     * basic constructor creates a projection with an associated color constraint;
     * @param index the projection index
     * @param exp the successor exponent
     * @param cc the projection color class
     */
    private Projection(final int index, final int succ, final ColorClass cc) {
        super(cc);
        this.index = index;
        this.succ = succ;
    }
    ...
    /**
     * creates a projection (successor) with an associated constraint the
     * successor argument (exp) may have to be simplified so that the following
     * holds: different keys map to different (according to equals) projections
     * @param index the projection index
     ...
     */
    public static final Projection builder(final int index, int succ, final ColorClass cc) {
        if (index < 1) {
        ...
        if (succ != 0 && !cc.isOrdered()) {
            ...
            final ComplexKey k = new ComplexKey(cc, index, succ);
            Projection p;
     ...
     */
    public static final Projection builder(final int index, final ColorClass cc) {
     ...
     */
    public Projection setExp(final int new_succ) {
     ...
     * @return the projection obtained by shifting <code>this</code> of h
     * positions
     */
    public Projection shift(final int h) {
     ...
     * @return the split-delimiter, taking into account the successor and the
     * color-class bounds
     */
    public static int succDelim(final int s, final ColorClass c) {
     ...
     */
    public static int maxSuccOffset(final Collection<? extends Projection> l, final ColorClass c) {
        if (c.isOrdered()) {
