...
/**
 * this class defines intersections over class functions; intersection is an
 * "AND" operator
 *
...
public final class Intersection extends N_aryClassOperator implements AndOp<SetFunction> {
    ...
    private Intersection(final Set<? extends SetFunction> args, final boolean check) {
        super(args, check);
    }
     ...
     */
    public static SetFunction factory(final Collection<? extends SetFunction> arglist, final boolean check) {
        final Set<? extends SetFunction> asSet = Util.asSet(arglist);
     ...
     * whether the collection's fixedSize is greater than or equal to one
     */
    public static SetFunction factory(final Collection<? extends SetFunction> arglist) {
     ...
     */
    public static SetFunction factory(final boolean check, final SetFunction... args) {
     ...
     * whether the list's fixedSize is greater than or equal to one
     */
    public static SetFunction factory(final SetFunction... args) {
        return factory(Arrays.asList(args), false);
    }

    /**
     * creates an intersection from a collection
     *
     * @param args a collection of set-functions
     * @return a (possibly simplified) Intersection
     */
    @Override
    public SetFunction buildOp(final Collection<? extends SetFunction> args) {
    ...
    /**
     * checks whether this intersection represents an "extended projection
     * complement"; also considers the lower bound of the constraints and the
     * min and max succ assumes that different projections map to different
     * colours and no duplicates are present
     *
    ...
    public Integer extendedCompl() {
        if (this.extended_compl == null) { // never computed before ...
            ...
            final Set<ProjectionBased> ncset = congruent(false);
            final int size = size(), ncsize = ncset.size();
            if (cc.isOrdered()) {
                if (size == ncsize) {
                    final Iterator<ProjectionBased> ite = ncset.iterator();
     ...
     * exclusively by ProjectionComp(s) and possibly one Subcl CAREFUL assumes
     * that the intersection-form is "simple" (constant-fixedSize), i.e., for
    ...
    public Interval card() {
        final Integer k = extendedCompl();
        if (0 != k) {
            if (this.card == null) {
                final Interval in = subclasses().isEmpty() ? getSort().card() : subclasses().iterator().next().card();
                final int ub = in.ub(), lb;
                final boolean bounded = !in.unbounded();
                if (bounded && ub <= k) {
    ...
    public Set<Integer> extendedComplSucc() {
        if (0 != extendedCompl()) {
            final HashSet<Integer> succset = new HashSet<>();
     ...
     * intersection
     */
    public HashSet<SetFunction> getComponents(final int i) {
     ...
     * single-indexed, pure generalised complement (defined on an ordered class);
    ...
    public Pair<SetFunction, Integer> baseCompose(final SetFunction right) {
        ...
        if (rcard != null) {
            ...
            if (rcard.singleValue(1)) { // base case: right's cardinality is one...
                ...
                final ColorClass cc = getSort();
                if (cc.isOrdered()) {
                    final Set<Integer> succargs = extendedComplSucc(); // the successor arguments...
                    ...
                    if (ecsize > 0) {
                        ...
                        if (cc.fixedSize() > 0) { // fixed card: the intersection of complements becomes a sum of projection succesors ...    
                            final Set<SetFunction> newargs = new HashSet<>();
     ...
     */
    public SetFunction join(final SetFunction other, final boolean check) {
        if (check && !this.getSort().equals(other.getSort())) {
            throw new IllegalDomain("cannot join intersections of different colors!");
        } else {
            final List<SetFunction> newargs = new ArrayList<>(getArgs());
    ...
    public Set<? extends Pair<? extends SetFunction, ? extends Guard>> toSimpleFunctions(final Set<? extends Equality> ineqlist, final Map<Projection, Subcl> inmap, final Map<Projection, Set<Subcl>> notinmap, Domain domain) {
        ...
        final Set< Pair<SetFunction, Guard>> res = new HashSet<>();
        Projection pr;
        final Set<? extends SetFunction> operands = getArgs();
        if (congruent(true).isEmpty()) {  // no Projection in the operands' list - only ProjectionComp(s) and (at most) one Subcl
            ...
            final Set<Subcl> subclasses = subclasses();
            ...
            if (!subclasses.isEmpty()) {  // there is one subclass
                ...
                for (SetFunction x : operands) {
                    final Subcl sc; // the (possiby null) subclass associated with the arg X_i (pr) of pc by an "in" clause
                    ...
                    final SetFunction redf = Intersection.factory(red_args);
            ...
            if (!reduced) { // we consider projection complements (with different indices)
                ...
                for (Map<Integer, Set<ProjectionBased>> ncm = Util.mapFeature(congruent(false), ProjectionBased::getIndex); ncm.size() > 1;) {
                    final Iterator<Map.Entry<Integer, Set<ProjectionBased>>> mit = ncm.entrySet().iterator();
                    final Map.Entry<Integer, Set<ProjectionBased>> e1 = mit.next(); //safe
                    do {
                        final Map.Entry<Integer, Set<ProjectionBased>> e2 = mit.next(); //safe
                        for (ProjectionComp pc1 : Util.cast(e1.getValue(), ProjectionComp.class)) {
                            for (ProjectionComp pc2 : Util.cast(e2.getValue(), ProjectionComp.class)) {
                                final Projection p1 = pc1.getArg(), p2 = pc2.getArg();
                                final Equality eq = (Equality) Equality.builder(p1, p2, false, domain);
            ...
            final Set<Guard> new_guards = new HashSet<>();
            final Iterator<ProjectionBased> ite = congruent(true).iterator(); // one projection is taken
     ...
     */
    private static boolean differentDom(final Projection p1, final Projection p2, final Map<Projection, Subcl> inmap, final Map<Projection, Set<Subcl>> notinmap) {
        final Subcl sc1 = inmap.get(p1), sc2 = inmap.get(p2);
