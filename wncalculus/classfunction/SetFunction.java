...
public abstract class SetFunction implements ClassFunction, SetExpr {
    ...
    public final boolean differentFromZero() {
        return card() != null && card().lb() > 0;
    }

    /**
     * @param ite a collection of class-functions mapping to sets
     * @return <code>true</code> if and only if every function is not equivalent
     * to "zero"
     */
    ...
    /**
     * @return <code>true</code> if and only if the size (cardinality) of
     * <code>this</code> function is less than or equal to one
     */
    public final boolean cardLeq1() {
        if (card() != null && card().singleValue(1)) {
            return true;
        }
        if (this instanceof Intersection intersection) {
            return Util.find(intersection.getArgs(), Projection.class) != null;
        }
        return false;   
    }

    /**
     * calculates the composition between type-set class-functions, disregarding
     * projection indices (i.e., considering functions as if they were "unary")
     * @param right the function to compose with <code>this</code>
     * @return a pair holding the composition result (<code>null</code> if, for
     * any reasons, the composition outcome cannot be derived) and an integer
     * representing a split delimiter required to solve the composition (in that
     * case a pair (null,intval) would be returned); if neither the composition
     * outcome nor a split delimiter can be devised a <code>null</code> pair is
     * returned
     */
    ...
    /**
     * puts a funaction to an equivalent set (sum) of simple (guarded)
     * class-functions this default version must be redefined if needed; it
     * assumes that basic reductions have been carried out; the arguments encode
     * the basic predicates (guard) associated with the tuple in which the
     * function is embedded Default implementation (to override)
     * @param ineqlist the inequalities
     * @param inmap a map describing the membership "in" predicates
     * @param domain the guard domain
     * @param notinmap a map describing the membership "notin" predicates (there
     * may be many, for a given variable)
     * @return if <code>this</code> is an "elementary" intersection-form, an
     * equivalent set (sum) of simple (guarded) class-functions, represented by
